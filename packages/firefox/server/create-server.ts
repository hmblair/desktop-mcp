...
import { z } from "zod";
import { FirefoxAPI } from "./browser-api";
...
export interface FirefoxMcpPlugin {
  register: (mcpServer: McpServer, browserApi: FirefoxAPI) => void;
}

export type FirefoxMcpServer = ManagedServer<FirefoxAPI>;

function registerResources(mcpServer: McpServer, browserApi: FirefoxAPI) {
  mcpServer.resource(
    "open-tab-contents",
...
export function createServer(options?: { name?: string; version?: string }): FirefoxMcpServer {
  return createSharedServer<FirefoxAPI>({
    name: options?.name ?? "firefox-mcp",
    version: options?.version,
    httpPort: HTTP_PORT,
    createApi: () => new FirefoxAPI(),
    registerTools: registerBuiltinTools,
    registerResources,
    pluginConfigName: "firefox-mcp",
  });
}
